...
b = csvread('b.csv',1,1)'   % [m #tp] matrix of m/z by timepoint
...
bfmrs_nn = bsxfun(@rdivide, bfmrs_nn, sum(bfmrs_nn,2)/100);     % normalize to sum=100
...
color = {'k', [0.5 0.5 0.5], 'k', [45, 171, 255]./255, [66, 134, 244]./255}
for i = 1:size(bfmrs_nn,2),
    hp = plot(bsxfun(@times,bfmrs_nn(:,i)*1e3,methane), style{i}, ...
        'color', color{i}, ...
        'MarkerEdgeColor', color{i}, ...
        'MarkerFaceColor', 'w')
    hold on
...
grid minor
set(gca, 'TickLength', [0.0200 0.0500])
ylabel('Calculated Concentration, mmoles/kg')
